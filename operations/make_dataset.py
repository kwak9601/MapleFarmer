...
def main(training_set_ratio):
    arrows = pd.DataFrame(
        np.zeros((1, 4), dtype=np.int32),  # np.zeros((3, 4), dtype=np.int32),
        index=('wide',),  # ('round', 'wide', 'narrow'),
        ...
        for t, _ in arrows.iterrows():
            ...
            for d in arrows:
                ...
                for path, filename in training:
                    dst_dir = os.path.join(common.TRAINING_DIR, d)
                    if not os.path.exists(dst_dir):
                        os.makedirs(dst_dir)

                    os.rename(path, os.path.join(dst_dir, filename))
                    os.rename(flipped(path), os.path.join(dst_dir, flipped(filename)))
                ...
                for path, filename in validation:
                    dst_dir = os.path.join(common.VALIDATION_DIR, d)
                    if not os.path.exists(dst_dir):
                        os.makedirs(dst_dir)
                ...
                for path, filename in testing:
                    dst_dir = os.path.join(common.TESTING_DIR, d)
                    if not os.path.exists(dst_dir):
                        os.makedirs(dst_dir)
...
def get_summary_matrix(directory):
    matrix = pd.DataFrame(
        np.zeros((2, 5), dtype=np.int32),  #np.zeros((4, 5), dtype=np.int32),
        index=('wide', 'total'),  # ('round', 'wide', 'narrow', 'total'),
